...
def run_sanity_check(test_dir):
    ...
    # FAIL_COLOR+f"No direcotry
    # named {test_dir} found in {os.getcwd()}"
    print('This script will perform a sanity test ')
    print('to ensure your code meets the criteria in the rubric.\n')
    print('Please enter the path to the file ')
    print('that contains your test cases for the GET() and POST() methods')
    ...
    if not test_functions_for_get:
        ...
        for func in test_functions_for_get:
            ...
            if (sf('.json') != -1) or (sf('json.loads') != -1):
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR + "Your test case \
                for GET() does not seem to be \
    ...
    if not test_functions_for_post:
        ...
        COUNT_POST = COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT
        if len(
                test_functions_for_post) >= 2 and COUNT_POST < 2:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(
                FAIL_COLOR +
